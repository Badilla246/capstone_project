<div class="font-bold bg-[#4D79F6] w-[20%] p-4 flex flex-col items-center user-select-none">
    <div class="flex flex-col text-md">
        <a href="dashboard" {{-- {{ request()->is('dashboard') ? 'bg-white w-full p-1 rounded text-[#4D79F6]' : 'p-1' }} --}}
            class="menu-link p-2 flex items-center gap-4 hover:rounded hover:text-slate-200 transition duration-100 ease-linear {{ request()->is('dashboard') ? 'text-slate-200' : '' }}">
        ...
        @can('create_user')
            <a href="user_management" {{--  {{ request()->is('user_management') ? 'bg-white w-full p-1 rounded text-[#4D79F6]' : 'p-1' }}  --}}
                class="menu-link  p-2 flex items-center gap-4  hover:rounded hover:text-slate-200 transition duration-100 ease-linear mt-2 {{ request()->is('user_management') ? 'text-slate-200' : '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="size-5">
                    <path stroke-linecap="round" stroke-linejoin="round"
